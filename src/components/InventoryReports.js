  ...
  const generatePDF = async () => {
    try {
      ...
      const [response1, response2, response3] = await Promise.all([
        axios.get('/api/inventory-turnover'),
        axios.get('/api/inventory-popular'),
        axios.get('/api/inventory-demands'),
      ]);

      // Extract data from responses
      const turnoverData = response1.data.turnoverData;
      const popularProductsData = response2.data.popularProductsData;
      const demandPredictionData = response3.data.demandPredictionData;
      ...
      pdf.setTextColor('#666');
      pdf.text(
        "This comprehensive report provides insights into the company's inventory management process, including turnover rates, popular products, and demand forecasts. These metrics help the company make informed decisions to optimize stock levels and meet customer demands effectively.",
        20,
        120,
      ...
      pdf.setTextColor('#333');
      pdf.text("In this report, you will find:", 20, 150);
      ...
      // Function to create chart images
